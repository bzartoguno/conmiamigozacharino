...
import re                  # Lets Python sort episode names in a more natural order
...
# Prefer QuickTime so the fullscreen, Picture-in-Picture, and speed hotkeys work.
# VLC is still used as a fallback if QuickTime cannot play a file.
...
print("Player plan:", "Use QuickTime Player first, fall back to VLC if QuickTime fails")
print("Hotkeys:", "backtick: stop program | |: skip clip | _: toggle fullscreen | }: toggle PiP | {: toggle 2x speed")
...
def ask_yes_no(question, default_yes=True):
    ...
    Pressing Enter uses the default answer.
    """
...
def natural_sort_key(path):
    """
    Sort file names naturally so episode 2 comes before episode 10.
    """
    file_name = os.path.basename(path).lower()
    return [int(piece) if piece.isdigit() else piece for piece in re.split(r"(\d+)", file_name)]


def choose_starting_episode(ordered_videos):
    """
    Show the ordered episode list and ask where playback should begin.
    Return the zero-based index to start from.
    """
    print("\nEpisodes found for this show:")

    for index, video in enumerate(ordered_videos, start=1):
        print(f"{index}. {os.path.basename(video)}")

    while True:
        raw_choice = input("\nWhich episode number should I start with? ").strip()
        ...
        if raw_choice.isdigit():
            ...
            if 1 <= number <= len(ordered_videos):
                return number - 1

        print(f"Please enter a number from 1 to {len(ordered_videos)}, or press Enter to start at 1.")
...
single_show_mode = isinstance(selected_tv_shows, list) and len(selected_tv_shows) == 1
single_show_break_with_movies = True
single_show_in_order = False
ordered_tv_index = 0
use_movies = True
...
if single_show_mode:
    print(f"\nSingle show selected: {selected_tv_shows[0]}")
    single_show_break_with_movies = ask_yes_no("Do you want this show broken up with movies?", default_yes=True)
    single_show_in_order = ask_yes_no("Do you want the episodes to play in order?", default_yes=True)
    use_movies = single_show_break_with_movies
...
def get_tv_to_movie_ratio():
    ...
    """
    if not use_movies or not movie_videos:
        return 1
...
movie_videos = get_videos(MOVIE_FOLDER) if use_movies else []
...
if use_movies:
    ...
    print("Movie playback disabled for this single-show run.")

# Stop immediately if one of the needed folders has no usable videos
if not tv_videos:
    print("Check your TV folder paths and ensure there are video files.")
    exit()

if use_movies and not movie_videos:
    print("Check your movie folder path and ensure there are video files, or choose not to break the show up with movies.")
    exit()

if single_show_in_order:
    tv_videos = sorted(tv_videos, key=natural_sort_key)
    ordered_tv_index = choose_starting_episode(tv_videos)

if use_movies:
    print("Dynamic TV-to-movie ratio:", get_tv_to_movie_ratio(), "TV clips per movie clip")
else:
    print("Single-show mode: episodes will play without movie breaks.")
...
def on_press(key):
    """Terminal hotkeys for the currently playing video.

    `  Stop the player and close the current video, but leave Terminal open.
    |  Skip the current clip.
    _  Toggle QuickTime fullscreen.
    }  Toggle QuickTime Picture-in-Picture.
    {  Toggle double-speed playback.
    """
    global stop_program, skip_current_video, current_process, current_player
    global quicktime_should_be_fullscreen, quicktime_should_be_picture_in_picture
    ...
    try:
        ...
        if key.char == '_' and current_player == "quicktime":
            ...
            if toggle_result == "NO_DOCUMENTS":
                ...
                print("Fullscreen preference is now", "ON" if quicktime_should_be_fullscreen else "OFF")
        ...
        if key.char == '}':
            ...
            if current_player == "quicktime":
                ...
                print(f"QuickTime PiP result: {pip_result}")
        ...
        if key.char == '{':
            ...
            if current_player == "quicktime":
                ...
                print(f"QuickTime speed result: {rate_result}")
            elif current_player == "vlc" and current_process:
...
def play_with_quicktime(video):
    ...
    # QuickTime occasionally resets a just-set rate while the movie is still loading.
    # Reapply it briefly so a chosen 2x setting sticks to every new clip.
    for _ in range(5):
        if stop_program:
            break
...
def play_video(video):
    """Play one clip, preferring QuickTime so the new hotkeys work."""
...
def choose_ordered_tv_video():
    """
    Pick the next TV episode in order, looping back to the start after the last episode.
    """
...
while not stop_program:
    ...
    tv_to_movie_ratio = get_tv_to_movie_ratio() if use_movies else 1
