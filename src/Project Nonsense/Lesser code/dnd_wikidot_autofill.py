...
Normal run:
    python3 dnd_wikidot_autofill.py
...
Process only the next 10 missing items:
    python3 dnd_wikidot_autofill.py --limit 10
...
def extract_description(session: requests.Session, url: str) -> str:
    """
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Process at most this many missing descriptions.",
    )
    ...
    if args.limit is not None:
        missing_rows = missing_rows[: max(0, args.limit)]
        print(f"Limit active: processing at most {len(missing_rows)} missing row(s).")
        print()
    ...
    for number, (row, item_name) in enumerate(missing_rows, start=1):
        ...
        try:
            description = extract_description(session, url)
